...
- **Описание**: Возвращает набор жанров для фильтрации на фронтенде. Каждый элемент содержит `id` и `name`.
- **Auth**: Нет
- **Response** (200):
```json
[
...
- **Примечания о полях**:
  - `country`: возвращается как строка с названием страны (например, "Россия"), а не как id.
  - `rating`: возвращается как числовое значение (float), представляющее средний рейтинг фильма.

### Оценки фильмов
**POST** `/api/v1/movies/<id>/rate/`
- **Описание**: Зарегистрированный пользователь может поставить или обновить оценку фильма (1-10).
- **Auth**: Да (Bearer или cookie)
- **Request** (JSON): `{ "score": 7 }` — целое число 1..10
- **Response** (200): `{ "movie": <id>, "rating": 7.0 }` — `rating` содержит обновлённый средний рейтинг фильма (float)
- **Ошибки**:
  - 400 Bad Request — некорректная оценка
